...
export function ReviewSection({ reviews, breakdown, onSubmit, onHelpful, isAuthenticated }: ReviewSectionProps) {
  ...
  return (
    <YStack gap="$3">
      <Text fontFamily="$body" fontSize="$5" fontWeight="700">Ratings & Reviews</Text>
      ...
      {count === 0 ? (
        <YStack backgroundColor="$surface" padding="$4" borderRadius="$3" borderWidth={1} borderColor="$borderColor" alignItems="center" gap="$2">
          ...
          <Text fontFamily="$body" color="$textSecondary">No reviews yet — be the first!</Text>
        </YStack>
      ) : (
        <YStack backgroundColor="$surface" padding="$3" borderRadius="$3" borderWidth={1} borderColor="$borderColor" gap="$2">
          <XStack alignItems="center" gap="$2">
            <Text fontFamily="$body" fontSize="$7" fontWeight="700">{avg.toFixed(1)}</Text>
            <YStack>
              <RatingStars rating={avg} reviewCount={count} />
              <Text fontFamily="$body" fontSize="$2" color="$textSecondary">{count} reviews</Text>
            </YStack>
          </XStack>
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <XStack key={star} alignItems="center" gap="$2">
                  ...
                  <Text fontFamily="$body" width={30} fontSize="$2" color="$textSecondary">{c}</Text>
                </XStack>
              );
            })}
          </YStack>
      ...
      <YStack gap="$2" backgroundColor="$surface" padding="$3" borderRadius="$3" borderWidth={1} borderColor="$borderColor">
        <Text fontFamily="$body" fontWeight="600">Write a review</Text>
        <XStack gap="$1">
          {[1, 2, 3, 4, 5].map((s) => (
            <Text fontFamily="$body" key={s} onPress={() => setRating(s)} fontSize="$5" color={s <= rating ? '$warning' : '$neutral300'} style={{ cursor: 'pointer' } as unknown as Record<string, unknown>}>
              ★
            </Text>
          ))}
        </XStack>
        ...
        {error && <Text fontFamily="$body" color="$error" fontSize="$2">{error}</Text>}
        <Button backgroundColor="$primary" onPress={handleSubmit} disabled={submitting} opacity={submitting ? 0.6 : 1}>
          <Text fontFamily="$body" color="white" fontWeight="600">{submitting ? 'Submitting...' : 'Submit review'}</Text>
        </Button>
        {!isAuthenticated && <Text fontFamily="$body" fontSize="$2" color="$textSecondary">Log in to submit.</Text>}
      </YStack>
      ...
      {/* List */}
      {reviews === undefined ? (
        <Text fontFamily="$body" color="$textSecondary">Loading reviews...</Text>
      ) : reviews.length === 0 ? (
        <Text fontFamily="$body" color="$textSecondary">No reviews yet.</Text>
      ) : (
        <YStack gap="$3">
          {reviews.map((r) => (
            <YStack key={r._id} backgroundColor="$surface" padding="$3" borderRadius="$3" borderWidth={1} borderColor="$borderColor" gap="$1">
              <XStack alignItems="center" justifyContent="space-between">
                ...
                <Text fontFamily="$body" fontSize="$2" color="$textSecondary">{new Date(r.createdAt).toLocaleDateString()}</Text>
              </XStack>
              <Text fontFamily="$body" fontSize="$3" color="$textPrimary">{r.text}</Text>
              <XStack alignItems="center" gap="$2" marginTop="$1">
                <Text fontFamily="$body" fontSize="$2" color="$textSecondary" onPress={() => onHelpful(r._id)} style={{ cursor: 'pointer' } as unknown as Record<string, unknown>}>
                  Helpful ({r.helpful}) ↑
                </Text>
              </XStack>
            </YStack>
