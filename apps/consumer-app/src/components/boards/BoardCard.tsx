  ...
  return (
    ...
    >
      <YStack height={100} backgroundColor="$backgroundHover" alignItems="center" justifyContent="center">
        {board.previewImage ? (
          <Image source={{ uri: board.previewImage }} src={board.previewImage} width={160} height={100} resizeMode="cover" style={{ objectFit: 'cover' }} />
        ) : (
          <Text fontFamily="$body" fontSize="$8" opacity={0.2}>♡</Text>
        )}
      </YStack>
      <YStack padding="$3" gap="$1">
        <Text fontFamily="$body" fontSize="$3" fontWeight="700" numberOfLines={1}>{board.name}</Text>
        <Text fontFamily="$body" fontSize="$2" color="$textSecondary">{board.itemCount} items</Text>
        <Text fontFamily="$body" fontSize="$1" color="$textTertiary" numberOfLines={1}>{board.slug}</Text>
      </YStack>
    </YStack>
  );
}
  ...
  return (
    ...
    >
      <YStack width={56} height={56} borderRadius="$2" backgroundColor="$backgroundHover" overflow="hidden" alignItems="center" justifyContent="center">
        {board.previewImage ? (
          <Image source={{ uri: board.previewImage }} src={board.previewImage} width={56} height={56} resizeMode="cover" style={{ objectFit: 'cover' }} />
        ) : (
          <Text fontFamily="$body" fontSize="$5" opacity={0.3}>♡</Text>
        )}
      </YStack>
      <YStack flex={1} gap="$1">
        <Text fontFamily="$body" fontWeight="600" numberOfLines={1}>{board.name}</Text>
        <Text fontFamily="$body" fontSize="$2" color="$textSecondary">{board.itemCount} items • {board.slug}</Text>
      </YStack>
      <Text fontFamily="$body" color="$primary" fontWeight="600" fontSize="$3">View</Text>
    </XStack>
  );
}
