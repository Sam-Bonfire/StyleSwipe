...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <YStack gap="$2" marginBottom="$4">
        <H2 fontSize="$7" fontWeight="700" color="$textPrimary">{title}</H2>
...
const ColorSwatch = ({ name, color }: { name: string; color: string }) => (
    <YStack alignItems="center" gap="$1" width={80}>
        ...
        />
        <Text fontSize="$2" color="$textSecondary" textAlign="center">{name}</Text>
        <Text fontSize="$1" color="$textTertiary">{color}</Text>
    </YStack>
);
...
const SpacingSwatch = ({ name, size }: { name: string; size: number }) => (
    <XStack alignItems="center" gap="$2" marginBottom="$1">
        <Text fontSize="$3" color="$textSecondary" width={60}>{name}</Text>
        <Stack height={16} width={size} backgroundColor="$primary" borderRadius="$1" />
        <Text fontSize="$2" color="$textTertiary">{size}px</Text>
    </XStack>
);
...
export function DesignSystemGallery() {
    ...
    const navItems = [
        { key: 'home', label: 'Home', icon: <Home size={22} color="$textSecondary" />, activeIcon: <Home size={22} color="$primary" /> },
        { key: 'discover', label: 'Discover', icon: <Search size={22} color="$textSecondary" />, activeIcon: <Search size={22} color="$primary" /> },
        { key: 'categories', label: 'Categories', icon: <Grid size={22} color="$textSecondary" />, activeIcon: <Grid size={22} color="$primary" /> },
        { key: 'account', label: 'Account', icon: <User size={22} color="$textSecondary" />, activeIcon: <User size={22} color="$primary" /> },
    ];
        ...
        <TamaguiProvider config={config}>
            <Theme name="BrandIdentityLight">
                <YStack flex={1} backgroundColor="$background">
                    {/* Top Bar Demo */}
                    <TopBar
                        addressValue="Koramangala, 560034"
                        wishlistCount={3}
                        cartCount={2}
                    />
                    ...
                    <ScrollView flex={1}>
                        <YStack padding="$3" gap="$4">
                            ...
                            <Section title="Color Palette">
                                <H3 fontSize="$5" color="$textSecondary">Brand Colors</H3>
                                ...
                                <H3 fontSize="$5" color="$textSecondary" marginTop="$2">Semantic Colors</H3>
                                <XStack flexWrap="wrap" gap="$2">
                                ...
                                <H3 fontSize="$5" color="$textSecondary" marginTop="$2">Neutral Scale</H3>
                                <XStack flexWrap="wrap" gap="$2">
                            ...
                            <Section title="Typography (Manrope)">
                                <Text fontSize="$12" fontWeight="700">Display Large</Text>
                                <Text fontSize="$9" fontWeight="700">Heading Large</Text>
                                <Text fontSize="$7" fontWeight="600">Heading</Text>
                                <Text fontSize="$5" fontWeight="400">Body Text</Text>
                                <Text fontSize="$3" fontWeight="400" color="$textSecondary">Label / Caption</Text>
                            </Section>
