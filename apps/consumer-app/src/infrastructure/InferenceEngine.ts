...
async function getOrLoadResources(): Promise<{ session: InferenceSession; tokenizer: PreTrainedTokenizer }> {
...
export async function generateEmbedding(text: string): Promise<Vector384> {
    try {
        ...
        const inputIds = new Tensor('int64', BigInt64Array.from(model_inputs.input_ids.data), model_inputs.input_ids.dims);
        const attentionMask = new Tensor('int64', BigInt64Array.from(model_inputs.attention_mask.data), model_inputs.attention_mask.dims);
        const tokenTypeIds = new Tensor('int64', BigInt64Array.from(model_inputs.token_type_ids.data), model_inputs.token_type_ids.dims);
