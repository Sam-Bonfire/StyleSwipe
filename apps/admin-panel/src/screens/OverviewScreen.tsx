        ...
        >
          ...
          <YStack space="$2">
            {stats.recentUsers.map((u: { _id: string; name?: string; email?: string }) => (
              <Text key={u._id} color="$color">
                New user joined: {u.name || u.email}
              </Text>
            ))}
          </YStack>
        </Card>
...
function StatsCard({ title, value, change }: { title: string; value: string; change: string }) {
  return (
    ...
    >
      <Text fontSize="$3" color="$color" textTransform="uppercase" fontWeight="bold">
        {title}
      </Text>
      <H2 marginVertical="$2" color="$color">
        {value}
      </H2>
      <Text fontSize="$3" color="$primaryLight">
        {change}
      </Text>
    </Card>
  );
