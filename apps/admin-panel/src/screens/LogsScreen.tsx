...
import { Button, useToast, Modal } from '@app/ui-kit';
import { ChevronDown, Filter, X, Search, RefreshCw, Info, Eye, Copy } from '@tamagui/lucide-icons';
...
export function LogsScreen() {
    ...
    const copyToClipboard = () => {
        if (fullData) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (Clipboard as any).setString(JSON.stringify(fullData, null, 2));
            showToast({
                title: 'Copied',
    ...
    const { results, status, loadMore, isLoading } = usePaginatedQuery(
        api.logs.getLogs,
        {
            filters: {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                level: filterLevel as any, // Cast for strict union type
        ...
        <YStack flex={1} padding="$4" space="$4">
            ...
            <YStack paddingHorizontal="$4" paddingVertical="$3" borderBottomWidth={1} borderColor="$borderColor">
                <XStack space="$4" alignItems="center" flexWrap="wrap">
                    ...
                    {/* Level Filter */}
                    <Select value={filterLevel || 'all'} onValueChange={(v) => setFilterLevel(v === 'all' ? undefined : v)}>
                                                ...
                                                {item.breadcrumbs && item.breadcrumbs.length > 0 && (
                                                    <YStack space="$2">
                                                        ...
                                                        <YStack space="$2" backgroundColor="$background" padding="$3" borderRadius="$4" borderWidth={1} borderColor="$borderColor">
                                                            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                                                            {item.breadcrumbs.map((crumb: any, i: number) => (
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function Badge({ children, backgroundColor, width, ..._rest }: { children: React.ReactNode; backgroundColor?: any, width?: any, [key: string]: any }) {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const dummy = _rest;
    return (
        <XStack
