...
export class MyntraAPIScraper {
  ...
  async scrapeCategory(
    ...
    console.log(
      `[MyntraAPIScraper] Starting scrape for path: ${path}, start: ${startPage}, pages: ${maxPages}`,
    );
    ...
    for (let pageNum = startPage; pageNum <= endPage; pageNum++) {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `[MyntraAPIScraper] Failed to fetch page ${pageNum}: ${response.status} ${response.statusText}`,
          );
