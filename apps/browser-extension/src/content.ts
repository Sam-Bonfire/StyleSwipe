...
function mapToScrapedProduct(data: MyntraRawData, embedding?: number[]): ScrapedProduct {
    ...
    return {
        externalId,
        url: window.location.href.includes(externalId) ? window.location.href : `https://www.myntra.com/${data.landingPageUrl || externalId}`,
        brand,
        title,
        price,
        mrp,
...
async function extractProductData(): Promise<ScrapedProduct | null> {
    try {
        ...
        try {
            ...
            const textToEmbed = `${data.name || data.productName} ${typeof data.brand === 'object' ? data.brand?.name : data.brand} ${data.description || ''} ${JSON.stringify(data.articleAttributes || {})}`.trim();
...
async function extractCategoryData(): Promise<ScrapedCategory | null> {
    ...
    try {
        ...
        console.log(`[StyleSwipe] Final batch size: ${products.length} (deduplicated from ${productsRaw.length})`);
    ...
    if (request.type === "SCRAPE_CATEGORY_PAGE") {
        extractCategoryData()
            .then(categoryData => {
                ...
                if (count > 0) {
                    chrome.runtime.sendMessage({ type: "SAVE_BATCH", data: categoryData!.products }, (response) => {
                        if (response?.success) {
                            chrome.runtime.sendMessage({
                                type: "SCRAPE_SUCCESS",
