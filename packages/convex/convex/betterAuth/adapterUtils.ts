...
const isUniqueField = (betterAuthSchema: Record<string, any>, model: string, field: string) => {
  let modelSchema = betterAuthSchema[model];

  // Workaround: If plural table names are used, the model name passed here (e.g. "users")
  // might not match the internal Better Auth model key (e.g. "user").
  // We look up the model by its modelName to ensure the correct schema is resolved.
  if (!modelSchema) {
    modelSchema = Object.values(betterAuthSchema).find((s: any) => s.modelName === model);
  }
  ...
  return Object.entries(fields)
    .filter(([, value]) => (value as any).unique)
    .map(([key]) => key)
    .includes(field);
};
...
export const checkUniqueFields = async <Schema extends SchemaDefinition<any, any>>(
  ...
  for (const field of Object.keys(input)) {
    ...
    const { index } =
      findIndex(schema, {
        model: table,
        where: [{ field, operator: 'eq', value: input[field as keyof typeof input] }],
      }) || {};
    if (!index) {
    ...
    const existingDoc = await ctx.db
      .query(table as any)
      .withIndex(index.indexDescriptor, (q) => q.eq(field, input[field as keyof typeof input]))
      .unique();
