...
describe('InMemoryQueueAdapter', () => {
    ...
    describe('push', () => {
        it('should return a unique ID', async () => {
            const id = await queue.push({ url: 'http://a.com' });
            expect(typeof id).toBe('string');
            expect(id.startsWith('mem-')).toBe(true);
        });

        it('should increment size', async () => {
            await queue.push({ url: 'http://a.com' });
            await queue.push({ url: 'http://b.com' });
    ...
    describe('pushBatch', () => {
        it('should push multiple items and return IDs', async () => {
            const ids = await queue.pushBatch([
                { url: 'http://a.com' },
                { url: 'http://b.com' },
                { url: 'http://c.com' },
            ]);
    ...
    describe('pull', () => {
        it('should return pending items', async () => {
            ...
            const items = await queue.pull(10);
        ...
        it('should respect batchSize limit', async () => {
            ...
            const items = await queue.pull(2);
            expect(items).toHaveLength(2);
        });
        ...
        it('should not return already-processing items', async () => {
            ...
            const secondPull = await queue.pull(10);
    ...
    describe('fail', () => {
        it('should re-queue failed items as pending', async () => {
            await queue.push({ url: 'http://a.com' });
            const [item] = await queue.pull(1);
            await queue.fail(item.id);
        ...
        it('should remove item after 3 retries', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                const pulled = await queue.pull(1);
                if (pulled.length > 0) {
                    await queue.fail(pulled[0].id, `attempt ${i + 1}`);
                }
            }
    ...
    describe('size', () => {
        ...
        it('should only count pending items', async () => {
            ...
            await queue.pull(1); // marks 1 as processing
