...
export type CouponInputProps = Omit<
    ...
    const handleApply = async () => {
      ...
      try {
        ...
        if (isValid) {
          ...
          setFeedback(
            validMessage ||
              `Coupon applied! You save ${discountAmount ? formatPrice(discountAmount as number) : ''}`,
          );
          setCode('');
        } else {
          setStatus('invalid');
          ...
          (
            <>
              <InputRow>
                ...
                <ApplyButton onPress={handleApply} disabled={!code.trim() || status === 'loading'}>
                  {status === 'loading' ? (
